    ...
    for e in xml.xpath(f".//{unit} | .//{anchor}"):
        if e.tag == unit:
            ...
            if "time" not in e.attrib:
                LOGGER.error(f'{anchor} element in {xml_filename} is missing the "time" attribute, ignoring it')
                continue
            stop = e.attrib["time"]
            if words:
                seqences.append(WordSequence(start, stop, words))
            words = []
            start = stop
    if words:
...
def align_audio(  # noqa: C901
    ...
    for i, word_sequence in enumerate(word_sequences):
        ...
        dict_data = make_dict(word_sequences.words, xml_path, unit=unit)
        if save_temps:
        ...
        fsg_data = make_fsg(word_sequence.words, xml_path, unit=unit)
        if save_temps:
